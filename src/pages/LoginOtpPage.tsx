/**
 * Login email OTP — required after password succeeds for verified accounts.
 */

import { useEffect, useMemo, useRef, useState, type FormEvent, type KeyboardEvent } from "react";
import { PublicHeader } from "../components/layout/PublicHeader";
import { ApiClientError } from "../services/apiClient";
import { resendLoginOtp, verifyLoginOtpAndPersistSession } from "../services/authService";
import {
  clearPendingLoginOtp,
  getPendingLoginOtpEmail,
  getPendingLoginOtpUsername,
  maskEmail,
  setPendingLoginOtp
...
export function LoginOtpPage({ onNavigate }: LoginOtpPageProps) {
  const [email, setEmail] = useState(() => getPendingLoginOtpEmail() ?? "");
  ...
  useEffect(() => {
    if (!email.trim()) {
      notify.validation("Sign in with your password first to receive a code.");
      onNavigate("/login");
    }
  }, [email, onNavigate]);
  ...
  const handleVerify = async (event: FormEvent) => {
    ...
    setError("");
    try {
      const result = await verifyLoginOtpAndPersistSession({
        email: email.trim(),
        otp: otpValue
      });
      const username = result.username?.trim() || getPendingLoginOtpUsername() || "";
      clearPendingLoginOtp();
      if (username) markPasswordLoginAdmin(username);
  ...
  const handleResend = async () => {
    ...
    setError("");
    try {
      const result = await resendLoginOtp({ email: email.trim() });
      setPendingLoginOtp(result.email || email, getPendingLoginOtpUsername());
      setEmail(result.email || email);
      setCooldown(result.resendAvailableInSeconds || 30);
      setDigits(Array.from({ length: OTP_LENGTH }, () => ""));
      notify.success("A new sign-in code has been sent.");
  ...
  return (
    <div className="min-h-full w-full overflow-y-auto bg-slate-950 text-slate-100">
      <PublicHeader pathname="/login/verify" />
      <div className="mx-auto w-full max-w-md px-4 py-8">
        <div className="rounded-2xl border border-slate-800 bg-slate-900/80 p-6 shadow-2xl shadow-black/30 backdrop-blur sm:p-7">
          <div className="mb-6 space-y-2 text-center">
            ...
            <h1 className="text-3xl font-semibold text-slate-100">Verify sign-in</h1>
            <p className="text-sm text-slate-400">
              We&apos;ve sent a verification code to:
              <br />
              <span className="font-medium text-slate-200">{masked}</span>
            </p>
            <p className="text-sm text-slate-400">Enter the 6-digit code to finish signing in.</p>
          </div>
